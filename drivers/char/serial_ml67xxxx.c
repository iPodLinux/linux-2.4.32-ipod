...
static void sio_irq(int irq, void *dev, struct pt_regs *regs)
{
	while (1) {
		...
		printk(KERN_INFO "sio_irq(%d,%p,%p) => %02x\n", irq, dev, regs, status);
...
static inline int sio_xmit(int ch)
{
	...
	printk("%s: ready %d, locked %d, xchar %02x\n",
